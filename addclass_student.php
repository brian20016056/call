...
if (isset($_GET['classId'])) {
  $colname_Recordset2 = $_GET['classId'];
}
mysql_select_db($database_memberConn, $memberConn);
$query_Recordset2 = sprintf("SELECT * FROM rollcalldata WHERE classId = %s AND rollcallCount =1", GetSQLValueString($colname_Recordset2, "text"));
...
if (isset($_GET['classId'])) {
  $colname_Recordset4 = $_GET['classId'];
}
mysql_select_db($database_memberConn, $memberConn);
$query_Recordset4 = sprintf("SELECT * FROM classdata WHERE classId = %s", GetSQLValueString($colname_Recordset4, "int"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "sander1")) {
	 for($x=1 ; $x<$row_Recordset4['classCount']+1 ; $x++)
				  { 
  $insertSQL = sprintf("INSERT INTO rollcalldata (classId, studentId, studentName, rollcallCount) VALUES (%s, %s, %s, %s)",
  ...
  $insertGoTo = "addclass_student.php?studentName=";
  if (isset($_SERVER['QUERY_STRING'])) {
              ...
              <table width="70%" border="1" cellpadding="1" cellspacing="0" bordercolor="#A4BAE0">
                ...
                <tr>
                  ...
                  <td align="center"><?php echo $row_Recordset1['studentNote']; ?></td>
                   ...
                   <?php if($row_Recordset1['studentName']==$row_Recordset2['studentName']){ ?>
                   <td align="center"> 
                   已加入
                   </td>
                   <?php }
                  ...
                  <form action="<?php echo $editFormAction; ?>" method="POST" name="sander1" id="rollcall">
                    <input type="hidden" value="<?php echo $_GET['classId']; ?>" name="classId" id="classId">
                    <input type="hidden" value="<?php echo $row_Recordset1['studentName']; ?>" name="studentName" id="studentId">
                    <input type="hidden" value="<?php echo $row_Recordset1['studentId']; ?>" name="studentId" id="studentId">
                    <input type="hidden" value="<?php echo $x; ?>" name="rollcallCount" id="rollcallCount">
                    <input  type="submit" value="加入" name="set">
...
<div class="課程資訊">
  <div align="center">課程學生
    <div>
      <table width="150px" border="1" cellpadding="1" cellspacing="0" bordercolor="#A4BAE0">
